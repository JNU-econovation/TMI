...
def educationToNum(x):
    x= np.char.lower(x)
    if np.char.find(x, "space") or np.char.find(x, "high school"):
        num_x= 1 
    elif np.char.find(x, "college"):
        num_x= 2 
    elif np.char.find(x, "univ") or np.char.find(x, "master"):
        num_x= 3
    elif np.char.find(x, "law") or np.char.find(x, "med") or np.char.find(x, "ph"):
        num_x= 4 
    else:
        num_x= 0 
    return num_x
